...
const SENDER = { name: 'Fibreglass Pool Check', email: 'deliver@send.fibreglasspoolcheck.com.au' }
const BRADY_EMAIL = 'help@fibreglasspoolcheck.com.au'
const SITE_URL = 'https://fibreglasspoolcheck.com.au'

// Google Drive links for digital products
const RED_FLAGS_GUIDE_LINK = 'https://drive.google.com/file/d/1eHT5IgroQFzGXJVpcQbyX7s-Hg3CX-t-/view?usp=sharing'
const BUYER_CHECKLIST_LINK = 'https://drive.google.com/file/d/1C7ZOf1iGtAtY4wFNPvKWU0pdIxnk2c9P/view?usp=sharing'
...
export async function sendRedFlagsGuide(firstName, email) {
  await sendEmail({
    ...
    htmlContent: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; color: #1e293b;">
        ...
        <p style="text-align: center; margin: 24px 0;">
          <a href="${RED_FLAGS_GUIDE_LINK}" style="background-color: #ea580c; color: white; padding: 12px 24px; border-radius: 8px; text-decoration: none; font-weight: bold;">Download Your Guide</a>
        </p>
...
export async function sendBuyerChecklistDelivery(fullName, email) {
  // Send to customer
  await sendEmail({
    ...
    htmlContent: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; color: #1e293b;">
        <h2 style="color: #0f172a;">Hi ${fullName.split(' ')[0]},</h2>
        <p>Thanks for your order. Here's your Fibreglass Pool Buyer Checklist:</p>
        <p style="text-align: center; margin: 24px 0;">
          <a href="${BUYER_CHECKLIST_LINK}" style="background-color: #ea580c; color: white; padding: 12px 24px; border-radius: 8px; text-decoration: none; font-weight: bold;">Download Your Checklist</a>
        </p>
...
export async function sendReportAcknowledgement(fullName, email, orderId) {
  ...
  await sendEmail({
    ...
    htmlContent: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; color: #1e293b;">
        <h2 style="color: #0f172a;">Thanks ${fullName.split(' ')[0]}, your order is confirmed.</h2>
...
export async function notifyBradyNewOrder(productName, fullName, email, orderId, details) {
  await sendEmail({
    ...
    htmlContent: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; color: #1e293b;">
        <h2 style="color: #0f172a;">New Order: ${productName}</h2>
        <table style="font-size: 14px; line-height: 1.8; border-collapse: collapse;">
          <tr><td style="padding-right: 16px; font-weight: bold;">Customer:</td><td>${fullName}</td></tr>
          <tr><td style="padding-right: 16px; font-weight: bold;">Email:</td><td><a href="mailto:${email}">${email}</a></td></tr>
          <tr><td style="padding-right: 16px; font-weight: bold;">Order ID:</td><td>${orderId}</td></tr>
          ${details ? `<tr><td style="padding-right: 16px; font-weight: bold;" valign="top">Details:</td><td>${details}</td></tr>` : ''}
        </table>
