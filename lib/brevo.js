...
// token param is first 12 chars of order/lead ID for basic verification
function getDownloadLink(product, tokenId) {
  const token = (tokenId || '').substring(0, 12)
  return `${SITE_URL}/api/download?product=${product}&token=${token}`
}
